...
var CalHeatMap = function() {
	...
	this._domainType = {
		"sec": {
			...
			position: {
				x: function (d) {
					return Math.floor(d.getSeconds() / self._domainType.sec.row(d));
				},
				y: function (d) {
					return d.getSeconds() % self._domainType.sec.row(d);
				}
			},
			format: {
				date: "%H:%M:%S, %A %B %-e, %Y",
	...
	this._init = function() {
        ...
        d3.select(self.options.itemSelector).append('a')
            .attr("href", "#")
            .attr("rel", "prev")
            .attr("class", "graph-browse-previous")
            .attr("title", "Load previous period")
            .append("i")
	...
	function w(d, outer) {
		var width = self.options.cellSize*self._domainType[self.options.subDomain].column(d) + self.options.cellPadding*self._domainType[self.options.subDomain].column(d) + 1;
...
CalHeatMap.prototype = {
	...
	init: function(settings) {
		...
		function validateDomainType() {
			if (!parent._domainType.hasOwnProperty(options.domain) || options.domain === "sec" || options.domain.substring(0, 2) === "x_") {
	...
	/**
	 * Return all the seconds between 2 dates
	 *
	 * @param  Date    d    date    A date
	 * @param  int|date    range    Number of seconds in the range, or a stop date
	 * @return array    An array of minutes
	 */
	getSecondDomain: function (d, range) {
		"use strict";
		...
		if (range instanceof Date) {
			stop = new Date(range.getFullYear(), range.getMonth(), range.getDate(), range.getHours(), d.getMinutes());
		} else {
			stop = new Date(+start + range * 1000);
		}
		console.log('stopped');
		console.log(stop);
		return d3.time.seconds(Math.min(start, stop), Math.max(start, stop));
	},
	 ...
	 * @return array	An array of minutes
	 */
	computeSecSubPeriodSize: function (d, range) {
		"use strict";
	...
	getDomain: function(date, range) {
        ...
        case "min" :
            return this.getMinuteDomain(date, range);
		case "hour" :
	...
	getSubDomain: function(date) {
         ...
         * @return int
         */
        var computeSecSubDomainSize = function (date, period) {
            switch (period) {
                case "min":
                    return 60;
                case "hour":
		...
		case "x_sec":
		case "sec"  :
			return this.getSecondDomain(date, computeSecSubDomainSize(date, this.options.domain));
		case "x_min":
		case "min"  :
			...
			if (updateMode === this.RESET_SINGLE_ON_UPDATE) {
                ...
                if (!isNaN(subDomainsData[index].v)) {
                    if (typeof data[d] === "boolean")
                        subDomainsData[index].v = Boolean(data[d]);
                    else
                        subDomainsData[index].v = data[d];
                        console.log(subDomainsData[index].t);
                        console.log(subDomainsData[index].v);
                } else {
                    subDomainsData[index].v += data[d];
                }
            }
...
Legend.prototype.redraw = function(width) {
	...
	legendItem.select("title").text(function(d, i) {
        if (i === 0) {
            return (options.legendTitleFormat.booltrue).format({
                min: options.legend[i],
                name: options.itemName[1]
            });
        } else if (i === _legend.length - 1) {
            return (options.legendTitleFormat.boolfalse).format({
                max: options.legend[i - 1],
                name: options.itemName[1]
            });
        } else {
            return (options.legendTitleFormat.unbool).format({
